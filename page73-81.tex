...
\begin{defin}
$\Lambda$ artin $R$-algebra. Then $\Lambda$ is \emph{basic}\index{algebra!basic} if \[\Lambda = P_1 \oplus P_2 \oplus \cdots \oplus P_n\] with $P_i$ indecomposable, then $P_i \not\simeq P_j$ for $i \neq j$.
\end{defin}
\begin{recall}
...
\emph{Morita equivalent}\index{Morita equivalent} if $\Mod\Lambda$ and $\Mod\Sigma$ are
